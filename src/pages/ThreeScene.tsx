...
const ThreeScene: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // 初始化時用 action 逐個添加，避免覆蓋 observable
        objects.forEach(obj => appStore.addThreeObject(obj));

        // 动畫循環，遍歷 appStore.threeObjects 並確保全部物件都在場景
        const animate = () => {
            requestAnimationFrame(animate);

            // 確保所有threeObjects都在scene.children
            if (Array.isArray(appStore.threeObjects)) {
    ...
    // 完善添加对象功能：隨機添加立方體、球體或圓柱
    const addRandomObject = () => {
        if (!sceneRef.current) return;
        console.log("add");
        const x = (Math.random() - 0.5) * 10; // 隨機x
        const y = Math.random() * 5 + 0.5;   // 隨機y
        const z = (Math.random() - 0.5) * 10; // 隨機z
        const color = Math.random() * 0xffffff; // 隨機顏色

        // 隨機選擇形狀
        const types = ['cube', 'sphere', 'cylinder'];
        const type = types[Math.floor(Math.random() * types.length)];
        console.log(`Adding ${type} at (${x.toFixed(2)}, ${y.toFixed(2)}, ${z.toFixed(2)}) with color ${color.toString(16)}`);
        let mesh;
        if (type === 'cube') {
            const geometry = new THREE.BoxGeometry(1, 1, 1);
            const material = new THREE.MeshLambertMaterial({ color });
    ...
    const clearObjects = () => {
        if (!sceneRef.current) return;
        // 使用MobX action方法
        appStore.clearThreeObjects(sceneRef.current);
    };
